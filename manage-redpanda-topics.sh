...
if [ -f redpanda.env ]; then
    set -a
    source redpanda.env
    set +a
else
    echo "Error: redpanda.env file not found"
    exit 1
fi
...
TOPICS=${REDPANDA_TOPICS:-iot_messages,alarms}
PARTITIONS=${REDPANDA_PARTITIONS:-1}
...
create_topics() {
    ...
    for i in {1..30}; do
        if docker exec redpanda rpk cluster health > /dev/null 2>&1; then
            echo "✅ Redpanda is ready!"
            break
        fi
        echo "⏳ Redpanda not ready yet... retrying in 2s ($i)"
        sleep 2
    done

    # Fail if Redpanda never became available
    if ! docker exec redpanda rpk cluster health > /dev/null 2>&1; then
    ...
    for topic in "${TOPIC_ARRAY[@]}"; do
        ...
        if ! docker exec redpanda rpk topic create "${topic}" \
            --partitions "${PARTITIONS}" \
            --replicas "${REPLICATION_FACTOR}" > /dev/null 2>&1; then
